...
def part_1(advent_of_code):
    with open('input.txt', 'r') as input_file:
        ...
        for pair in input_file:
            first, second = pair.replace("\n", "").split(",")
            first, second = convert(first), convert(second)
            if first.issubset(second) or second.issubset(first):
                fully_containing_pairs += 1

        advent_of_code.answer(1, fully_containing_pairs)


def convert(sections) -> set:
...
def part_2(advent_of_code):
    with open('input.txt', 'r') as input_file:
        ...
        for pair in input_file:
            first, second = pair.replace("\n", "").split(",")
            first, second = convert(first), convert(second)
            if len(first.intersection(second)) > 0 or len(second.intersection(first)) > 0:
                overlapping_pairs += 1

        advent_of_code.answer(2, overlapping_pairs)
